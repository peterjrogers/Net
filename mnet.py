...
class Mnet(Tools):
    ...
    def mt_ping(self, ip_list, count=1, timeout=300, ttl=255, size=32):    #ping test
    ...
    def mt_trace(self, ip, ttl=30, timeout=300, size=32):    #traceroute - send all ttl values in parrallel
        self.ping_count = 1
        self.ping_timeout = timeout
        self.ping_size = size
        self.ip = ip
    ...
    def trace_view(self):    #view trace results and return dict
        ...
        for key in self.out: 
            ...
            out[hop] = [res]
            if str(self.ip) in str(reply): return out
